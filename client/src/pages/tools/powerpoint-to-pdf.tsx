...
export default function PowerPointToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert('PowerPoint file processed, but full PDF conversion requires additional setup.');
      } else {
        alert('PowerPoint to PDF conversion: ' + result.message);
      }

    } catch (error) {
      console.error('PowerPoint conversion error:', error);
      alert('Failed to convert PowerPoint: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      clearInterval(progressInterval);
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4" data-testid="page-title">PowerPoint to PDF</h1>
        ...
        <Alert className="mb-6">
          ...
          <AlertDescription>
            PowerPoint to PDF conversion requires specialized server setup. For best results, consider using Microsoft Office Online or dedicated conversion services.
          </AlertDescription>
        </Alert>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Select a PowerPoint presentation (.pptx or .ppt) to convert to PDF.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div
            ...
            >
              ...
              {isDragActive ? (
                ...
                <div>
                  <p className="text-lg mb-2">Drag & drop a PowerPoint presentation here, or click to select</p>
                  <p className="text-sm text-muted-foreground">Supports .pptx and .ppt files • Maximum file size: 10MB</p>
                </div>
              )}
            </div>
